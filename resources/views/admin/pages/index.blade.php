<x-admin-layout title="Pages - Admin">
    <main class="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <h2 class="text-3xl font-bold text-gray-800 dark:text-white">Pages Management</h2>
            <a href="{{ route('admin.pages.create') }}" class="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90">
            ...
            <form method="GET" action="{{ route('admin.pages.index') }}">
                ...
                <div class="flex flex-wrap gap-4 mt-4">
                    ...
                    @if(request()->hasAny(['status', 'sort', 'search']))
                        <a href="{{ route('admin.pages.index') }}" class="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-background-dark">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider" scope="col">Slug</th>
                    ...
                    @forelse($pages as $page)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                <a href="{{ route('admin.pages.show', $page) }}" class="hover:text-primary">
                                    {{ $page->title }}
                                </a>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-4">
                                    <a href="{{ route('admin.pages.show', $page) }}" class="text-primary hover:underline">
                                        <span class="material-symbols-outlined align-middle text-sm">visibility</span>
                                        View
                                    </a>
                                    <a href="{{ route('admin.pages.edit', $page) }}" class="text-primary hover:underline">
